...
:mkiMaxJoystickAxis(100)
{
	...
	if(lpdikeyboard)
	{
		err=lpdikeyboard->SetCooperativeLevel(hwnd, DISCL_FOREGROUND|DISCL_NONEXCLUSIVE/*|DISCL_EXCLUSIVE*/);
		if (FAILED(err))
		{
	...
	if(lpdikeyboard)
	{
		...
		{
			// Failed to set cooperative level
			ErrorLogger::Writeln("Failed to acquire keyboard\n");
			ErrorLogger::Writeln(ErrorString(err));
			lpdikeyboard->Release();
			lpdikeyboard=NULL;
		}
	...
	memset(mrgcKeystate, 0, sizeof(mrgcPrevKeystate));
	...
	// Setting cooperation level to BACKGROUND (gets input whether on top or not)
	//   and EXCLUSIVE ( don't let other programs use the mouse, too)
	if(lpdimouse)
	{
...
void MyInputs::SampleMouse()
{
	...
	{
		mbMouseLeft = true;
		OutputDebugString("Left is down\n");
	}
	else
	{
		mbMouseLeft = false;
...
unsigned char* MyInputs::GetKeyboardState()
{
	return mrgcKeystate;
}
...
void MyInputs::SampleKeyboard()
{
	...
	{
		static HKL layout=GetKeyboardLayout(0);		// Get the layout of the current keyboard

		if(layout==NULL)
			ErrorLogger::Writeln("No keyboard layout");

		// Store the old key state
		std::memcpy(mrgcPrevKeystate, mrgcKeystate, sizeof(mrgcKeystate));
		...
		if(FAILED(err))
		{
			...
			if (err==DIERR_INPUTLOST || err==DIERR_NOTACQUIRED)
			{
				...
				err=lpdikeyboard->GetDeviceState(sizeof(mrgcKeystate), (LPVOID) &mrgcKeystate);
				if(FAILED(err))
					ErrorLogger::Writeln("Failed");
				else
					ErrorLogger::Writeln("Success");
		...
		// Search for a new button.
		// Wonder if I can do this by checking other bits than the high-value? Probably not.
		// This would be easier with GetAsyncKeyState, which returns taps as well as up/down
		for(UINT i=0;i<256;i++)
		{

			if( (mrgcKeystate[i]&0x80)>0 && !((mrgcPrevKeystate[i]&0x80)>0) )	// Pressed this time, but not last time
			{
				NewScanCode=i;
				break;
			}
		...
		UINT virtualKey=MapVirtualKeyEx(NewScanCode, 1, layout);	// Translate scan code to virtual key

		// Translating to an unshifted character
		mcNewKey = MapVirtualKeyEx(virtualKey, 2, layout);		// Translate virtual key to a character

		// Translating to a full character. Doesn't work for q and w, though. Why not?

		//unsigned short rgcKeyValue[2];
		//ToAsciiEx(virtualKey, NewScanCode ,mrgcKeystate,rgcKeyValue,0,layout);
		//mcNewKey = (unsigned char)rgcKeyValue[0];
	}
	else
		ErrorLogger::Writeln("No keyboard to sample");
...
char MyInputs::GetNewKeyInput()
	{

		if(mcNewKey == mcLastKey) return 0;
		mcLastKey = mcNewKey;
		if(mcNewKey>127||mcNewKey<0) return 0;
		return mcNewKey;
	}
