...
// Vesion 10.1 13/3/06
//	Fixed keyboard re-sample on SampleKeyboard (was reacquiring mouse, not keyboard)
//  Fixed use of unsigned char, not char on keystate array
// Vesion 10.2 3/4/06
//	Added GetNewKeyInput() - a bit hacky, but it does a useful job.
...
private:
	...
	unsigned char mrgcKeystate[256];	// Array to hold the keyboard state. (Usually 102 keys, but 256 allows
										//		for future expansion
	unsigned char mrgcPrevKeystate[256];	// Array to hold the keyboard state from the previous sample. (Usually 102 keys, but 256 allows
										//		for future expansion
	unsigned char mcLastKey;			// Key that was down on the last press, so as to not return when checking for new keys
	unsigned char mcNewKey;						// Stores any new key that has been pressed since last run of SampleKeyboard
...
public:
	...
	// Returns the character of a single new keypress since the last sample of the keyboard
	// Returns 0 if no key has been pressed.
	// If more that one key has been pressed, the function will return one of them, and
	// the other will be lost as a "new" press.
	// Keys with no obvious character are ignored (such as cursor keys)
	// If either shift button was held down, the UK-keyboard shift character will be returned.
	// This is a rudimentary function for simple typed input.
	char GetNewKeyInput();
